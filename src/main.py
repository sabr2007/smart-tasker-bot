...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_greeting_only(text):
        await update.message.reply_text(
            ...
            "🚀 С чего начать? Прежде чем мы начнем, очень советую заглянуть в Инструкцию (в настройках WebApp). Там я показываю, как добавлять 10 задач за одну минуту и управлять ими в одно касание.\n\n"
            "Жми кнопку ниже! 👇",
            reply_markup=MAIN_KEYBOARD,
        )
        return
